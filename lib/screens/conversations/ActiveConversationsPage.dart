...
import 'package:connect/logic/conversations/ActiveConversations.dart';
...
class ActiveConversationsPage extends StatefulWidget {
  final int currentIndex;

  final Function onChangeIndex;
  final Function updateParent;

  const ActiveConversationsPage({Key key, this.currentIndex, this.onChangeIndex,this.updateParent})
      : super(key: key);

  @override
  _ActiveConversationsPageState createState() => _ActiveConversationsPageState();
}

class _ActiveConversationsPageState extends State<ActiveConversationsPage> {
  ...
  void _onRefresh() async{
    ...
    ActiveConversations.fetchNotifications(update);
     ActiveConversations.updateUserInformation();
  ...
  Widget build(BuildContext context) {
    ...
    bool isArchived (ContactUser user) {
      if (LocalData.exitsString("conversation_archived")) {
        List<ContactUser> archivedUsers = ContactUser
            .decode(LocalData.getString("conversation_archived"));

        List<String> archivedUsersIds = [];

        for(int i = 0; i < ContactUser.decode(LocalData.getString("conversation_archived")).length; i++) {
      ...
      Widget buildListView() {
        if(LocalData.exitsString("conversation_list")) {
          if (ContactUser
              .decode(LocalData.getString("conversation_list"))
              .length != 0) {
            return Expanded(
        ...
        return Expanded(child: SmartRefresher(  enablePullDown: true,
          enablePullUp: false,
